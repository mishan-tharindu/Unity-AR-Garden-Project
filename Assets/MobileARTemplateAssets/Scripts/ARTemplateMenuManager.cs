...
public class ARTemplateMenuManager : MonoBehaviour
{
    ...
    public TouchObjectDetector touchObjectDetector;
    ...
    void Start()
    {
        ...
        // If not assigned in the Inspector, try to find it dynamically
        if (touchObjectDetector == null)
        {
             touchObjectDetector = Object.FindObjectOfType<TouchObjectDetector>();
            if (touchObjectDetector == null)
            {
                Debug.LogError("TouchObjectDetector not found in the scene!");
            }
        }
    ...
    void HandleClick(Vector2 inputPosition)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (selectedObject != null && selectedObject.CompareTag("ARObject"))  // ✅ Prevents NullReferenceException
            {
                //Debug.Log("Object Clicked: " + selectedObject.name);
                CallTouchObject(selectedObject);
                
            }
            else
            {
                // If the clicked object is not tagged as "ARobject", check its parent
                if (selectedObject.transform.parent != null && selectedObject.transform.parent.CompareTag("ARObject"))
                {
                    //GameObject parentObject = selectedObject.transform.parent.gameObject; // Assign parent object
                    GameObject childObject = selectedObject.transform.gameObject; // Assign parent object
                    Debug.Log("ARObject Clicked (Child): " + childObject.name);
                    CallTouchObject(childObject);
                }
                else
                {
                    Debug.LogWarning("Clicked object is not an ARObject or a child of an ARObject.");
                }
            }
        }
        else
        {
            Debug.LogWarning("No object detected on click.");
        }
    }

    void CallTouchObject(GameObject obj) {

        if (obj == null)
        {
            Debug.LogError("CallTouchObject() received a null object!");
            return;
        }

        if (touchObjectDetector == null)
        {
            Debug.LogError("touchObjectDetector is not assigned!");
            return;
        }

        Debug.Log("CallTouchObject() called on: " + obj.name);
        // Add your logic here (e.g., highlight object, open UI, etc.)
        touchObjectDetector.getSelectedObject(obj);
    ...
    void DeleteFocusedObject()
    {
        ...
        {
            Debug.LogWarning("No focused object to delete or change material.");
            return;
        }

        //Hide UI
        selectedMenuUI.HideSelectedObjectUI();

        // Get the focused object
        var currentFocusedObject = m_InteractionGroup.focusInteractable;
        Debug.Log("Delete Object :: " + currentFocusedObject.transform.name);


        //// Find the "Visuals" child GameObject
        //Transform visualsChild = currentFocusedObject.transform.Find("Visuals");
        //if (visualsChild == null)
        //{
        //    Debug.LogWarning("No 'Visuals' child found on the focused object: " + currentFocusedObject.transform.name);
        //    return;
        //}

        //// Get the Renderer component from the "Visuals" child
        //Renderer renderObject = visualsChild.GetComponent<Renderer>();
        //if (renderObject == null)
        //{
        //    Debug.LogWarning("No Renderer component found on the 'Visuals' child: " + currentFocusedObject.transform.name);
        //    return;
        //}

        //// Change the material
        //renderObject.material = red;
        //Debug.Log("Material changed to red for object: " + currentFocusedObject.transform.name);
